        ...
        public void CollectBiomassMortality(ISpecies species, int age, double mortality_wood, double mortality_nonwood, int AboveBelow)
        {
            ...
            if (AboveBelow == 0)        //aboveground wood
            {
                ...
                if (PlugIn.ModelCore.CurrentTime == 0)
                    SoilVars.BioInput[(int)ComponentType.FOLIAGE, idxSpecies, age] += nonwoodC_mort;
                ...
                if (mortality_wood > 0)
                {
                    ...
                    if (PlugIn.ModelCore.CurrentTime == 0)
                    {
                        SoilVars.BioInput[(int)ComponentType.MERCHANTABLE, idxSpecies, age] += woodC_mort * PropStem;
                        SoilVars.BioInput[(int)ComponentType.OTHER, idxSpecies, age] += woodC_mort * (1 - PropStem);
                    }
                }
            }
            ...
            {                           //belowground wood
                ...
                if (PlugIn.ModelCore.CurrentTime == 0)
                {
                    SoilVars.BioInput[(int)ComponentType.FINEROOT, idxSpecies, age] += nonwoodC_mort;
                    SoilVars.BioInput[(int)ComponentType.COARSEROOT, idxSpecies, age] += woodC_mort;
                }
            }
            ...
            for (int j = 0; j < 1; j++)
            {
                ...
                if (bPrintFlux)
                {
                    for (i = 0; i < NUMBIOMASSCOMPONENTS; i++)
                    {
                        ...
                        {
                            if (j == 0) 
                                logFlux.Write("{0:0.000},", netCLoss[i, species.Index]);     //SEPT: netCLoss = litterInput
                            netCLoss[i, species.Index] = 0.0;     //SEPT: moved from DoSoilDynamics
                        }
                    }
                    logFlux.WriteLine("");
